...
$('#add_row').on('click', function(){
    ...
    row_html = '<tr><td class="loop">'+sno+'</td><td class="p-0"><textarea class="reg_doc_no" required></textarea></td>'+
    '<td class="p-0"><textarea class="doc_type" required></textarea></td>'+
    '<td class="p-0"><textarea class="cliamant_name" required></textarea></td>'+
    '<td class="p-0"><textarea class="executant_name" required></textarea></td>'+
    ...
    '<td class="p-0"><textarea class="district" required></textarea></td>'+
    '<td class="p-0"><i style="font-size:24px" class="fa delete">&#xf014;</i></td></tr>'
    $('#prohibited_table').append(row_html)
})
...
$(document).on('click', '.delete', function() {
    ...
    s_no = 1
    $('#prohibited_table tr').each(function(){
...
function save_data(){
    ...
    $('#prohibited_table tr').each(function(){
        ...
        reqdocno = td.children('.reg_doc_no').val()
        doctype = td.children('.doc_type').val()
        claimantname = td.children('.cliamant_name').val()
        executantname = td.children('.executant_name').val()
        surveyno = td.children('.survey_no').val()
        ...
        temp.push({'reqdocno': reqdocno, 'doctype': doctype, 'claimantname': claimantname,
        'executantname': executantname, 'extent': extent, 'mandal': mandal, 'district': district, 'id': id,
        'surveyno': surveyno, 'village': village, 'sno': sno, 'requestid': requestid})
    })
    if($('#save').length){
        final_data = {
    ...
    $.post("http://localhost:8000/save-encumbrance-details", final_data, function (res) {
...
//        console.log(file.name.split('.'))
        ...
        var fd = new FormData();
        data = {'requestid': requestid, 'task': 'E', 'filename': file.name, 'filetype': filetype}
        fd.append('file', file)
...
function getScreens(){
    $.get("http://localhost:8000/get-screens?requestid="+requestid+"&task=E", function (res) {
        display_screens(res)
    });
}
