...
$('#add_row').on('click', function(){
    ...
    row_html = '<tr><td class="loop">'+sno+'</td><td class="p-0"><textarea  class="village" required></textarea></td>'+
    ...
    '<td class="p-0"><textarea class="natureofland" required></textarea></td>'+
    '<td class="p-0"><select class="landclassification" required><option selected disabled></option>'+
    '<option value="P">Patta</option><option value="G">Govt</option><option value="A">Assigned</option>'+
...
function save_data(){
    ...
    $('#prohibited_table tr').each(function(){
        ...
        pattadar = td.children('.pattadar').val()
        survey_no = td.children('.survey_no').val()
        extent = td.children('.extent').val()
        natureofland = td.children('.natureofland').val()
        landclassification = td.children('.landclassification').val()
        id = null
        if(this.id){
            id = this.id.replace('tr', '')
        }
        temp.push({'village': village, 'pattadharname': pattadar, 'surveyno': survey_no, 'extent': extent,
        'natureofland': natureofland, 'landclassification': landclassification, 'sno': sno,
    ...
    $.post("http://localhost:8000/save-preliminary-details", final_data, function (res) {
...
//        console.log(file.name.split('.'))
        ...
        var fd = new FormData();
        data = {'requestid': requestid, 'task': 'P', 'filename': file.name, 'filetype': filetype}
        fd.append('file', file)
...
function getScreens(){
    $.get("http://localhost:8000/get-screens?requestid="+requestid+"&task=P", function (res) {
        display_screens(res)
    });
}
