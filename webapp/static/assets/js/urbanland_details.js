...
function save_data(){
    ...
    $('#prohibited_table tr').each(function(){
        ...
        answer = td.children('.answer').val()
        id = null
        if(this.id){
            id = this.id.replace('tr', '')
        }
        temp.push({'response': answer, 'requestid': requestid, 'qid': id})
    })
    if($('#save').length){
        final_data = {
    ...
    $.post("http://localhost:8000/save-urban-details", final_data, function (res) {
...
//        console.log(file.name.split('.'))
        ...
        var fd = new FormData();
        data = {'requestid': requestid, 'task': 'U', 'filename': file.name, 'filetype': filetype}
        fd.append('file', file)
...
function getScreens(){
    $.get("http://localhost:8000/get-screens?requestid="+requestid+"&task=U", function (res) {
        display_screens(res)
    });
}
