...
$('#claimant').on('change', function(){
    if($(this).val() == 'N'){
        ...
        $('td textarea').attr('required', false)
        $('td select').attr('required', false)
    }else{
        $('#table').attr('hidden', false)
        $('#add_row').attr('hidden', false)
        $('td textarea').attr('required', true)
        $('td select').attr('required', true)
    }
})
...
$('#add_row').on('click', function(){
    ...
    row_html = '<tr><td class="loop">'+sno+'</td><td class="p-0"><textarea class="casenumber" required></textarea></td>'+
    '<td class="p-0"><textarea class="courtname" required></textarea></td>'+
    '<td class="p-0"><textarea class="petitioners" required></textarea></td>'+
    '<td class="p-0"><textarea class="defendants" required></textarea></td>'+
    '<td class="p-0"><select class="casestatus" required><option disabled selected></option>'+
    '<option value="D">Disposed</option><option value="P">Pending</option></select></td>'+
    '<td class="p-0"><textarea class="ordernumber" required></textarea></td>'+
    '<td class="p-0"><i style="font-size:24px" class="fa delete">&#xf014;</i></td></tr>'
    $('#legalcase_table').append(row_html)
})
...
$(document).on('click', '.delete', function() {
    ...
    s_no = 1
    $('#legalcase_table tr').each(function(){
...
function save_data(){
    ...
    $('#legalcase_table tr').each(function(){
        ...
        casenumber = td.children('.casenumber').val()
        courtname = td.children('.courtname').val()
        petitioners = td.children('.petitioners').val()
        defendants = td.children('.defendants').val()
        casestatus = td.children('.casestatus').val()
        ordernumber = td.children('.ordernumber').val()
        id = null
        if(this.id){
            id = this.id.replace('tr', '')
        }
        temp.push({'casenumber': casenumber, 'courtname': courtname, 'petitioners': petitioners, 'requestid': requestid,
        'defendants': defendants, 'casestatus': casestatus, 'ordernumber': ordernumber, 'id': id, 'sno': sno})
    })
    if($('#save').length){
        final_data = {
    ...
    $.post("http://localhost:8000/save-legalcase-details", final_data, function (res) {
...
//        console.log(file.name.split('.'))
        ...
        var fd = new FormData();
        data = {'requestid': requestid, 'task': 'L', 'filename': file.name, 'filetype': filetype}
        fd.append('file', file)
...
function getScreens(){
    $.get("http://localhost:8000/get-screens?requestid="+requestid+"&task=L", function (res) {
        display_screens(res)
    });
}
